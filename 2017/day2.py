...
1311	64	77	1328	1344	1248	1522	51	978	1535	1142	390	81	409	68	352'''.replace('\t',' ').split('\n')

def part1():
    total_dif = 0
    for row in input:
        digits = row.split(' ') # create a list for the row with each value split by space
        digits = [int(i) for i in digits] # convert each number to integer
        max_val = max(digits)
        min_val = min(digits)
